...
const getCommitEvent = () => {
  ...
  // UrlFetchでデータを取得して、JSONデータをパースする
  const response = JSON.parse(UrlFetchApp.fetch(url, options))
  const commited_id = cache.get(cache_key)
  if (commited_id === null) cache.put(cache_key, response[0].id)
  ...
  if (commited !== 0) {
    ...
    for (let i in events) {
      ...
      if (patch !== null) {
        ...
        const prskBot = 'https://script.google.com/macros/s/AKfycbxZemVFuAyXLUStVYXTFabJ3qt9_upebNreatLPRPffdxAY9hHWXTYzIySnj7LEXaXKZQ/dev'
        const headers = {
...
const putCache = () => {
  CacheService.getScriptCache().put('sekai_db_latest_commit_id', '21898967020')
}
